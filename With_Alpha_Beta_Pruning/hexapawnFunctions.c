...
bool checkMovePlayer(int start_row, int start_column, int end_row, int end_column) {
    if (checkPawnOwnership(HUMAN, start_row, start_column) == 0) {
...
char checkWinner() {
    ...
    if (!humanHasMoves) return COMPUTER; // Human has no moves, computer wins
        ...
        for (int j = 0; j < 3; j++) {
            if (board[i][j] == COMPUTER) {
                if (i + 1 < 3 && board[i + 1][j] == ' ') {
                    board[i + 1][j] = COMPUTER;
                    board[i][j] = ' ';
                    int eval = minimax(0, false, INT_MIN, INT_MAX);
                    board[i][j] = COMPUTER;
                    board[i + 1][j] = ' ';
                    if (eval > bestEval) {
                        bestEval = eval;
